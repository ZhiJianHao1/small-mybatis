...
import com.zhi.mybatis.datasource.unpooled.UnpooledDataSource;
 ...
 * 有连接池的数据源
 * @author zhijianhao
 * @created by zhijianhao on 2023/3/26-18:11
 */
public class PooledDataSource implements DataSource {
    ...
    // 给连接池一个打印日志状态机会的低层次设置，还有重新尝试获得连接，这些情况下往往需要很长时间，为了避免连接池没有设置时静默失败
    protected int poolTimeToWait = 20000;
    // 发送到数据的侦测查询，用来验证连接是否正常，并且准备 接受请求默认是 NO PING QUERY SET 这会引起许多数据库驱动连接由一个错误信息而导致失败
    ...
    public void pushConnection(PooledConnection connection) throws SQLException {
        synchronized (state) {
            ...
            //判断连接是否有效
            logger.info("connection.isValid ->  判断连接是否有效");
            ...
            synchronized (state) {
                ...
                // 如果无空闲连接 创建新的连接
                    ...
                    if (state.activeConnections.size() < poolMaximumActiveConnections) {
                        ...
                        logger.info("Created connection " + conn.getRealConnection() + ".");
                    }
                    // 活跃连接数已满
                    else {
                        // 取得活跃连接列表的第一个，也就是最老的一个连接
                        logger.info("活跃连接已满  -> 取得活跃列表的第一个,也就是最老的一个连接");
                        PooledConnection oldestActiveConnections = state.activeConnections.get(0);
                        long longestCheckoutTime = oldestActiveConnections.getCheckoutTime();
                        // 如果checkout 时间过长，则这个连接标记为过期
                        if (longestCheckoutTime > poolMaximumCheckoutTime) {
                            state.claimedOverdueConnectionCount++;
                            state.accumulatedCheckoutTimeOfOverdueConnections += longestCheckoutTime;
                            state.accumulatedCheckoutTime += longestCheckoutTime;
                            state.activeConnections.remove(oldestActiveConnections);
                            if (!oldestActiveConnections.getRealConnection().getAutoCommit()) {
                                oldestActiveConnections.getRealConnection().rollback();
                            }
                            // 删掉最老的连接，然后重新实例化一个新的连接
                            conn = new PooledConnection(oldestActiveConnections.getRealConnection(), this);
                            oldestActiveConnections.invalidate();
                            logger.info("Claimed overdue connection " + conn.getRealConnection() + ".");
                        }
                        // 如果checkout 超时时间不够长 等待
                        else {
                            try {
                                if (!countedWait) {
                ...
                if (conn != null) {
                    if (conn.isValid()) {
                        ...
                        // 设置连接的类型码。该类型码是根据数据库连接的 URL、用户名和密码生成的字符串，
                        // 用于标识数据库连接的唯一性，以避免连接池中出现重复连接。
    ...
    public boolean pingConnection(PooledConnection conn) {
        ...
        try {
            /**
             * !conn.getRealConnection().isClosed(); 连接池中的数据库是否关闭
             */
        ...
        if (result) {
            /**
             * poolPingEnabled 开启或禁用侦测
             */
            if (poolPingEnabled) {
                /**
                 * poolPingConnectionsNotUsedFor 用来配置 poolPingQuery 多次时间被用一次
                 * conn.getTimeElapsedSinceLastUse() 当前系统时间减去 poolPingQuery 多次时间被用一次
                 */
                if (poolPingConnectionsNotUsedFor >= 0 && conn.getTimeElapsedSinceLastUse() > poolPingConnectionsNotUsedFor) {
                    try {
                        logger.info("Testing connection " + conn.getRealHashCode() + "...");
                        /**
                         * 1.获取一个真正数据连接 connection
                         * 2.使用该连接对象创建 Statement 对象，以执行 SQL 查询或更新操作
                         * 3.验证数据库连接是否可用，并在使用完连接后将其还回连接池。
                         *
                         */
    ...
    @Override
    public void setLoginTimeout(int longTimeout) throws SQLException {
        DriverManager.setLoginTimeout(longTimeout);
    }
