...
import com.zhi.mybatis.reflection.SystemMetaObject;
...
public class BeanWrapper extends BaseWrapper {
    ...
    public Object get(PropertyTokenizer prop) {
        // 如果有index(有中括号)，说明集合，那就要解析集合，调用的是 BaseWrapper.resolveCollection 和 getCollectionValue
    ...
    public void set(PropertyTokenizer prop, Object value) {
        //如果有 index， 说明是集合，那就要解析集合，调用的是 BaseWrapper.resolverCollection 和 setCollectionValue
        if (prop.getIndex() != null) {
            Object collection = resolveCollection(prop, object);
    ...
    public boolean hasSetter(String name) {
        ...
        if (prop.hasNext()) {
            if (metaClass.hasSetter(prop.getIndexedName())) {
                MetaObject metaValue = metaObject.metaObjectForProperty(prop.getIndex());
                if (metaValue == SystemMetaObject.NULL_META_OBJECT) {
